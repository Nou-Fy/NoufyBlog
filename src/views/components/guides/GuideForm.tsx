...
export default function GuideForm({
  ...
  const fieldClasses =
    "w-full h-10 px-3 border border-stone-200 rounded-xl outline-none transition-all focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-slate-700 text-sm bg-white flex items-center";

  return (
    <div className="w-full p-6 bg-white rounded-2xl relative shadow-xl border border-stone-100 max-h-[90vh] overflow-y-auto">
      {onClose && (
        <button
          onClick={onClose}
          type="button"
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            <label className="text-sm font-semibold text-slate-400 flex items-center gap-1">
              <Link2 className="w-3 h-3" /> Slug
            </label>
            <input
              {...register("slug")}
              readOnly
              className={`${fieldClasses} bg-stone-50 text-slate-400 italic cursor-not-allowed ${isEditing ? "opacity-50" : ""}`}
            />
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            {...register("content")}
            rows={5}
            className="w-full p-3 border border-stone-200 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-sm min-h-[120px]"
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end pb-8">
          ...
          <div className="space-y-1.5">
            ...
            <Select
              ...
              <SelectContent
                position="popper"
                className="bg-white z-[9999] shadow-xl border border-stone-100">
                {GUIDE_ICONS.map((ico) => {
